...
public class algsimV14 {
    ...
    static Integer delScling = 100;// 每个时间间隔的切片请求数
    static int nodeBound = 169;
    static int linkBound = 169;

    public static void main(String[] arg) {
        int[] cap = {
                36, 49, 64, 81, 100, 110, 121, 144, 169, 225,
                36, 49, 64, 81, 100, 110, 121, 144, 169, 144,
                36, 49, 64, 81, 100, 110, 121, 144, 169, 144,
                36, 49, 64, 81, 100, 110, 121, 144, 169, 121,
                36, 49, 64, 81, 100, 110, 121, 144, 169, 121
        };
        Arrays.sort(cap);
//        System.out.println(cap);
        ...
        for (int i = 0; i < I; i++) {
            for (int j = i; j < I; j++) {
                if (C.get(i * H + 1) > 35 && C.get(i * H + 1) < 145) {
                    B[i][j] = (int) ((C.get(i * H + 1) + C.get(j * H + 1)) * H / 50);///////////////////////
                    B[j][i] = B[i][j];
                } else {
                    B[i][j] = (int) ((C.get(i * H + 1) + C.get(j * H + 1)) * H / 50);///////////////////////
                    B[j][i] = B[i][j];
    ...
    public static ArrayList<ArrayList<Float>> test() {
        ...
        for (int t = 0; t < delT; t++) {
            for (int scling = 0; scling < delScling; scling++) {
                ...
                mapResult2 = BestFit.process(K, I, H, C2, B2, node_req, link_req); // 方法2
                mapResult3 = RR.process(K, I, H, C3, B3, node_req, link_req);
                for (int k = 0; k < K; k++) {
