...
public class algsimV15 {
    ...
    static Integer delScling = 200;// 每个时间间隔的切片请求数
...
//        System.out.println(cap);
        ...
        for (int i = 0; i < I; i++) {
            for (int j = i; j < I; j++) {
                B[i][j] = (int)((C.get(i * H + 1) + C.get(j * H + 1))*H/100);///////////////////////
                B[j][i] = B[i][j];
            }
        }
//设置一些骨干带宽
    ...
    public static ArrayList<ArrayList<Float>> test() {
        ...
        for (int t = 0; t < delT; t++) {
            for (int scling = 0; scling < delScling; scling++) {
                ...
                K = SliceGenerate1.Generate(node_req, link_req, nodeBound, linkBound, Kmax);
