...
/**
 * @see 计算链路资源目标归一化的目标函数值
 * @author fb
 *
 */
public class algsimV9 {
	...
	static Integer delScling = 20;// 每个时间间隔的切片请求数
	static int nodeBound = 300;
	...
	public static ArrayList<ArrayList<Float>> test() {
		...
		for (int t = 0; t < delT; t++) {
			...
			result.get(0).add(linkResourceUsed1.floatValue() / currentLinkReq.floatValue());

			result.get(1).add(linkResourceUsed2.floatValue() / currentLinkReq.floatValue());

			result.get(2).add(linkResourceUsed3.floatValue() / currentLinkReq.floatValue());
